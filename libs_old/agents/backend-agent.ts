...
export class BackendAgent extends BaseAgent {
    ...
    async execute(input: { prompt: string, schema?: string, isIncremental?: boolean, affectedFiles?: string[] }, _context: AgentContext, signal?: AbortSignal, strategy?: any): Promise<AgentResponse> {
        ...
        try {
            ...
            const { result, tokens } = await this.promptLLM(system, `Project: ${input.prompt}\nSchema: ${input.schema || 'No explicit schema provided'}`, 'llama-3.3-70b-versatile', signal, strategy);
