...
type PayloadCallback = (payload: RealtimePostgresChangesPayload<any>) => void;
        ...
        if (!channelRecord) {
            ...
            newChannel.on('postgres_changes', binding, (payload) => {
                ...
                if (status === 'SUBSCRIBED') {
                    ...
                    console.error(`[RealtimeManager] ${channelName} connection error.`, err);
        ...
        return () => {
            ...
            if (record) {
                ...
                if (record.refCount <= 0) {
                    console.log(`[RealtimeManager] Scheduling teardown for ${channelName} in 1500ms`);
