import { RuntimeRecord, RuntimeMetrics, RuntimeCapacity, RuntimeHeartbeat } from '@packages/utils';
/**
 * rollingRestart.ts
 *
...
export const RollingRestart = {
    ...
    async start(): Promise<RollingRestartState> {
        ...
        try {
            ...
            nodes.sort((a: any, b: any) => a.runningRuntimes - b.runningRuntimes);
    ...
    async waitForDrain(nodeId: string): Promise<void> {
        ...
        while (Date.now() < deadline) {
            ...
            const nodeAssignments = await Promise.all(
                allRecords
                    .filter((r: any) => r.status === 'RUNNING' || r.status === 'STARTING')
                    .map(async (r: any) => {
            ...
            await new Promise((r: any) => setTimeout(r, DRAIN_CHECK_INTERVAL));
        }
