...
import { NodeRegistry, NODE_PREFIX } from './nodeRegistry';
...
export const FailoverManager = {
    ...
    async detectDeadNodes(): Promise<string[]> {
        if (!redis) {
            logger.error('[FailoverManager] Redis client is undefined. Skipping node detection.');
            return [];
        }

        try {
            const registeredIds = await redis.smembers('cluster:nodes');
            const activeNodes = (await redis.keys(`${NODE_PREFIX}*`)).map((k: string) => k.replace(NODE_PREFIX, ''));
            const dead: string[] = [];
            ...
            logger.error({ err }, '[FailoverManager] Failed to detect dead nodes');
            return [];
        }
    },
    ...
    async detectDeadWorkers(): Promise<string[]> {
        if (!redis || !missionController) {
            logger.error('[FailoverManager] Redis or MissionController undefined. Skipping worker detection.');
            return [];
        }
