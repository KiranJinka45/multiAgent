...
export function SidebarProvider({ children }: { children: React.ReactNode }) {
    ...
    useEffect(() => {
        ...
        // Also set the CSS variable on the root document for global layout adjustments
        const root = document.documentElement;
        if (isCollapsed) {
            root.style.setProperty('--sidebar-width', '64px'); // Collapsed width
        } else {
...
export function useSidebar() {
    ...
    if (!context) {
        throw new Error("useSidebar must be used within a SidebarProvider. Wrap your app in <SidebarProvider> in app/layout.tsx.");
    }
    return context;
}
