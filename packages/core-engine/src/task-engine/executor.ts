...
import { StrategyEngine, eventBus, patchEngine, DistributedExecutionContext, AgentMemory } from '@packages/utils';
import { logger } from '@packages/observability';
import { DebugAgent, AgentResponse } from '@packages/brain';
...
export class TaskExecutor {
    ...
    async evaluateGraph(graph: TaskGraph, globalContext: AgentContext) {
        let isRunning = true;
        const projectId = globalContext.projectId || (globalContext.getProjectId ? globalContext.getProjectId() : 'unknown');
        ...
        if (cycle) {
            ...
            await eventBus.error(globalContext.executionId || (globalContext.getExecutionId ? globalContext.getExecutionId() : 'unknown'), `Circular dependency detected: ${cyclePath}`, projectId);
            return;
        }
        ...
        try {
            while (isRunning) {
                ...
                const currentCompleted = graph.getAllTasks().filter((t: any) => t.status === 'completed').length;
                const running = graph.getAllTasks().filter((t: any) => t.status === 'running');
                ...
                if (running.length > 0 && (Date.now() - lastProgressTime > STALL_THRESHOLD)) {
                    logger.warn({ runningTasks: running.map((t: any) => t.id) }, 'STALL DETECTED: No node completion. Initiating recovery...');
    ...
    private async executeTask(task: BaseTask, graph: TaskGraph, globalContext: AgentContext) {
        ...
        try {
            ...
            const res = await this.workerPool.run(async () => {
    ...
    private async syncTaskResults(res: AgentResponse<unknown>, globalContext: AgentContext) {
        ...
        if (Array.isArray(files)) {
            const projectId = globalContext.getProjectId?.() || 'unknown';
