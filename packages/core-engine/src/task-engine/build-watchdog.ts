...
export class BuildWatchdog {
    ...
    private async check() {
        ...
        const runningTasks = this.graph.getAllTasks().filter((t: any) => t.status === 'running');
        ...
        if (runningTasks.length === 0 && !this.graph.isCompleted() && !this.graph.hasFailed()) {
            const ready = this.graph.getAllTasks().filter((t: any) => t.status === 'ready' || t.status === 'waiting');
            if (ready.length === 0) {
                ...
                this.graph.getAllTasks().forEach((t: any) => {
