...
import { AgentRequest, AgentResponse, AgentContext } from './types/local-contracts';
import { agentRegistry } from './task-engine/agent-registry';
import { projectMemory, redis } from '@packages/utils';
...
export class WorkflowEngine {
    ...
    async run(tasks: WorkflowTask[], context: AgentContext): Promise<AgentResponse[]> {
        ...
        while (state.status === 'running') {
            const pending = state.tasks.filter((t: any) => !state!.results[t.id]);
            if (pending.length === 0) {
            ...
            const ready = pending.filter((t: any) => 
                t.dependencies.every((d: string) => state!.results[d]?.success)
            );

            if (ready.length === 0) {
                const failedDeps = pending.filter((t: any) => t.dependencies.some((d: string) => state!.results[d]?.success === false));
            ...
            await Promise.all(ready.map(async (task) => {
                ...
                while (attempt < maxRetries && !success) {
                    ...
                    try {
                        ...
                        const response = await agent.execute(request, context, new AbortController().signal);
                        const duration = Date.now() - start;

                        lastResponse = response;

                        // Track Metrics
                        state!.metrics.totalTokens += response.metrics?.tokensTotal ?? 0;
                        ...
                        if (lastResponse && lastResponse.success) {
                            success = true;
                            await eventBus.thought(context.executionId, 'supervisor', `Iteration ${attempt} complete. Tokens: ${lastResponse.metrics?.tokensTotal || 0}`);
                        } else {
