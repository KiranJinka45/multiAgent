...
import { SandboxPoolManager } from './sandbox-pool.js';
import { SnapshotLibrary } from './snapshot-library.js';
import { AdmissionController } from './admission-controller.js';
import { MicroVMManager } from './microvm-manager.js';
...
export class PreviewServerManager {
    ...
    async checkHealth(previewId: string): Promise<boolean> {
        ...
        if (!reg) return false;
        return this.isHttpReady(reg.port ?? 0);
    }
    ...
    async startServer(projectId: string): Promise<string> {
        ...
        if (!previewId) {
            ...
            await PreviewRegistry.update(projectId, { port });
