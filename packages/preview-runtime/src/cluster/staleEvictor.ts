...
export const StaleEvictor = {
    ...
    async evict(record: RuntimeRecord, reason: EvictionReason): Promise<void> {
        ...
        try {
            ...
            await redis.setex(
                `${EVICTION_REASON_PREFIX}${projectId}`,
                86400, // 24h
                JSON.stringify(event)
            );

            await RuntimeMetrics.recordCrash(projectId, reason);
    ...
    async preemptLowestPriority(): Promise<string | null> {
        const allRecords = await PreviewRegistry.listAll();
        const running = allRecords.filter((r: any) => r.status === 'RUNNING');

        if (running.length === 0) return null;

        const now = Date.now();

        const scored = running.map((r: any) => {
