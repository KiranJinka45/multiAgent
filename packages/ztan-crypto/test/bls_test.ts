...
async function testBls() {
    ...
    const { masterPublicKey, shares } = await ThresholdBls.dkg(3, 5);
    ...
    const partialSignatures = await Promise.all(
        signers.map(s => ThresholdBls.signShare(payload, s.secretShare))
    );
    ...
    const aggregatedSignature = await ThresholdBls.aggregate(partialSignatures);
    ...
    const isValid = await ThresholdBls.verify(
        aggregatedSignature,
        payload,
        signers.map(s => s.verificationKey)
    );
    ...
    const isValidWrong = await ThresholdBls.verify(
        aggregatedSignature,
        payload,
        wrongPublicKeys
    );
