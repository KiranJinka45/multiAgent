...
async function runTest() {
    ...
    const p1 = Frost.generateRound1(t, n);
    const p2 = Frost.generateRound1(t, n);
    const p3 = Frost.generateRound1(t, n);

    console.log("Step 2: DKG Round 2 (Share Exchange)...");
    // Node 1 receives shares
    const sk1 = Frost.aggregateShares([
        Frost.computeShareForNode(p1.coeffs, 1),
        Frost.computeShareForNode(p2.coeffs, 1),
        Frost.computeShareForNode(p3.coeffs, 1)
    ]);
    // Node 2 receives shares
    const sk2 = Frost.aggregateShares([
        Frost.computeShareForNode(p1.coeffs, 2),
        Frost.computeShareForNode(p2.coeffs, 2),
        Frost.computeShareForNode(p3.coeffs, 2)
    ]);

    const masterPk = Frost.computeMasterPublicKey([p1.commitments, p2.commitments, p3.commitments]);
    const vk1 = Frost.computeVerificationKey(sk1);
    const vk2 = Frost.computeVerificationKey(sk2);
    const vk3 = Frost.computeVerificationKey(Frost.aggregateShares([
        Frost.computeShareForNode(p1.coeffs, 3),
        Frost.computeShareForNode(p2.coeffs, 3),
        Frost.computeShareForNode(p3.coeffs, 3)
    ]));
    ...
    const finalMsg = Buffer.from(sha256(bindingPayload));
    ...
    const sig1 = await Frost.computeBlsPartialSignature(sk1, finalMsg, S, 1);
    const sig2 = await Frost.computeBlsPartialSignature(sk2, finalMsg, S, 2);
