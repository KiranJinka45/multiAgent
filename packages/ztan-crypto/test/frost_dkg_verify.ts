...
async function runTest() {
    ...
    const masterPk = Frost.computeMasterPublicKey(allCommitments);
    console.log(`Master PK: ${masterPk}`);

    // 2. ROUND 2: Share Exchange
    // Node 1 receives shares from everyone at index 1
    const s1_1 = Frost.computeShareForNode(p1.coeffs, 1);
    const s2_1 = Frost.computeShareForNode(p2.coeffs, 1);
    const s3_1 = Frost.computeShareForNode(p3.coeffs, 1);
    
    // Node 1 verifies shares
    const v1 = VSS.verifyShare(s1_1, 1, p1.commitments);
    const v2 = VSS.verifyShare(s2_1, 1, p2.commitments);
    const v3 = VSS.verifyShare(s3_1, 1, p3.commitments);
    ...
    const sk1 = Frost.aggregateShares([s1_1, s2_1, s3_1]);
