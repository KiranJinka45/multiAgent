...
import { createLazyProxy } from '@libs/utils/server/runtime';
...
export const DEPLOYMENT_QUEUE = 'deployment-queue-v1';
export const redis = createLazyProxy(() => RedisClient.getInstance(), 'Redis_Index');
export const independentRedisClients = createLazyProxy(() => 
    REDIS_URLS.length > 1 ? RedisClient.getIndependentClients() : [redis], 
    'IndependentRedisClients_Index'
);
