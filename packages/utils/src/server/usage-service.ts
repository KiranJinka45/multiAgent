...
export class UsageService {
    ...
    static async recordAiUsage(metric: UsageMetric): Promise<void> {
        ...
        try {
            ...
            pipeline.hincrby(`usage:tenant:${metric.tenantId}`, 'totalTokens', metric.totalTokens);
            pipeline.hincrby(`usage:tenant:${metric.tenantId}`, 'apiCalls', 1);
            pipeline.expire(`usage:tenant:${metric.tenantId}`, 60 * 60 * 24 * 32); // 32 days TTL
            await pipeline.exec();

            // 2. DB Persistence (Transaction for consistency)
            await prisma.$transaction([
                // Update overall tenant usage summary
                prisma.usage.upsert({
