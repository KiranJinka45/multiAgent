...
exports.CostGovernanceService = exports.quotaEngine = exports.regionalGovernance = exports.RegionalGovernanceService = exports.RegionalStatus = exports.QuotaEngine = void 0;
...
class QuotaEngine {
    ...
    async getTenantLimits(tenantId) {
        try {
            const subscription = await db_1.db.subscription.findFirst({
                where: { tenantId },
                select: { plan: true }
            });
            ...
            observability_1.logger.error({ err, tenantId }, '[QuotaEngine] Failed to fetch tenant limits');
     ...
     */
    async reserveExecutionSlot(tenantId) {
        ...
        `;
        const result = await server_1.redis.eval(luaScript, 2, activeJobsKey, dailyLimitKey, limits.maxConcurrentJobs, limits.dailyExecutionLimit);
        return {
            allowed: result[0] === 1,
     ...
     */
    async decrementActiveJobs(tenantId) {
        const activeJobsKey = `governance:active_jobs:${tenantId}`;
        const current = await server_1.redis.get(activeJobsKey);
        if (current && parseInt(current, 10) > 0) {
            await server_1.redis.decr(activeJobsKey);
            await server_1.redis.decr('governance:total_active_jobs');
        }
    }
    /**
    ...
    async isKillSwitchActive() {
        const val = await server_1.redis.get('governance:kill_switch');
        return val === 'true';
    }
    async checkAndIncrementExecutionLimit(userId) {
    ...
    async checkRateLimit(tenantId) {
        ...
        const count = await server_1.redis.incr(key);
        if (count === 1) {
            await server_1.redis.expire(key, windowSize + 10); // add 10s buffer for safety
        }
    ...
    async checkUserRateLimit(userId) {
        ...
        const count = await server_1.redis.incr(key);
        if (count === 1) {
            await server_1.redis.expire(key, windowSize + 10);
        }
        if (count > userLimit) {
            observability_1.logger.warn({ userId, count }, '[Governance] User rate limit exceeded');
            return { allowed: false, retryAfter: windowSize };
        }
        return { allowed: true };
    }
    async trackUsage(tenantId, tokens) {
        const key = `governance:token_usage:${tenantId}:${new Date().getMonth()}`;
        await server_1.redis.incrby(key, tokens);
    }
    /**
    ...
    async getSystemLoad() {
        ...
        const totalActive = await server_1.redis.get('governance:total_active_jobs');
    ...
    async checkSystemLoad(options = {}) {
        const { isOverloaded, score } = await this.getSystemLoad();
        const priority = options.priority || 'low';
        // Hard Threshold: Reject everything
        if (score > 1000) {
            observability_1.logger.error({ score }, '[Governance] CRITICAL_OVERLOAD: Rejecting all requests');
        ...
        if (isOverloaded && priority === 'low') {
            observability_1.logger.warn({ score }, '[Governance] LoadShedding: Rejecting low-priority request');
    ...
    async getMetrics() {
        return {
            totalActiveJobs: await server_1.redis.get('governance:total_active_jobs'),
...
(function (RegionalStatus) {
    RegionalStatus["HEALTHY"] = "HEALTHY";
    RegionalStatus["DEGRADED"] = "DEGRADED";
    RegionalStatus["FAILING"] = "FAILING";
    RegionalStatus["OFFLINE"] = "OFFLINE";
})(RegionalStatus || (exports.RegionalStatus = RegionalStatus = {}));
class RegionalGovernanceService {
     ...
     */
    async getRouteAffinity(missionId) {
    ...
    async reportHeartbeat(metrics) {
        ...
        await server_1.redis.set(key, JSON.stringify(payload), 'EX', 30); // 30s TTL
        await server_1.redis.hset('governance:global_topology', this.currentRegion, status);
    }
    /**
     * Checks if a target region is available.
     */
    async isRegionHealthy(region) {
        const status = await server_1.redis.hget('governance:global_topology', region);
     ...
     */
    async getFailoverRegion() {
        const topology = await server_1.redis.hgetall('governance:global_topology');
...
exports.RegionalGovernanceService = RegionalGovernanceService;
exports.regionalGovernance = new RegionalGovernanceService();
exports.quotaEngine = new QuotaEngine();
exports.CostGovernanceService = exports.quotaEngine;
