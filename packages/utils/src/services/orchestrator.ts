...
import { getExecutionLogger, logger } from '../config/logger';
...
export class StageStateMachine {
    ...
    async transition(stage: string, state: StageState, message: string, progress: number) {
...
export class Orchestrator {
    async run(
        ...
        try {
            ...
            await fsm.transition('PLANNING', StageState.RUNNING, 'Orchestrating Temporal mission...', 10);
            ...
            await fsm.transition('COMPLETE', StageState.COMPLETED, 'Project ready via Temporal!', 100);
            ...
            if (fsm) await fsm.transition('FAILED', StageState.FAILED, errorMsg, 0);
