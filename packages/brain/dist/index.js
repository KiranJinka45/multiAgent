...
__export(index_exports, {
  ...
  ProgrammaticSeoAgent: () => ProgrammaticSeoAgent,
...
${prompt2}`;
    try {
      ...
      import_utils.default.error({ error, agent: this.getName() }, "Failed to generate architecture blueprint");
      return {
        success: false,
...
var EvolutionAgent = class extends BaseAgent {
  ...
  async run(input, _context, signal, strategy) {
    ...
    const patterns = await import_db.db.pattern.findMany({
      where: { type: "failure_pattern" },
      orderBy: { weight: "desc" },
      take: 3
    });
    const modules = await import_db.db.codeModule.findMany({
    ...
    try {
      ...
      const proposal = await import_db.db.proposedChange.create({
        data: {
