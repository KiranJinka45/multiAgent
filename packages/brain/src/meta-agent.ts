...
export class MetaAgent extends BaseAgent {
    ...
    protected async run(input: { task: string, availableAgents: string[] }, _context: AgentContext, signal?: AbortSignal, strategy?: any): Promise<AgentResponse> {
        const { task } = input;
...
Output strictly valid JSON matching this schema:
{
  ...
  "recommendedTechStack": {
    ...
    "styling": "string"
            ...
            const { result, tokens } = await this.promptLLM(system, prompt, 'llama-3.3-70b-versatile', signal, undefined, _context);
