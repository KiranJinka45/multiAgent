...
import { logger, redis, getSafeEnv } from '@packages/utils';
...
export class PreviewServerManager {
    ...
    constructor() {
        ...
        // Initialize Phase 12-15 infrastructure
    ...
    async launchPreview(projectId: string, framework = 'nextjs'): Promise<string> {
        ...
        // ADMISSION CONTROL: Ensure host capacity before starting (Phase 13)
    ...
    async checkHealth(projectId: string): Promise<boolean> {
        ...
        if (!reg || !reg.ports || reg.ports.length === 0) return false;
        return this.isHttpReady(reg.ports[0]);
    }
    ...
    async startServer(projectId: string): Promise<string> {
        let registryEntry = await PreviewRegistry.get(projectId);
        if (!registryEntry) {
            const executionId = 'legacy-' + projectId;
            registryEntry = await PreviewRegistry.init(projectId, executionId);
            const port = await this.portAllocator.allocate();
            await PreviewRegistry.update(projectId, { ports: [port] });
        }

        const reg = await PreviewRegistry.get(projectId);
        if (!reg || !reg.ports || reg.ports.length === 0) throw new Error('Preview registration missing or invalid');

        const previewDir = path.join(process.cwd(), '.generated-projects', projectId);
        const port = reg.ports[0];
    ...
    private startHeartbeatMonitor(projectId: string, child: ChildProcess) {
        ...
        const timer = setInterval(async () => {
            ...
            // Check HTTP health
            const isHealthy = await this.checkHealth(projectId);
            if (!isHealthy) {
