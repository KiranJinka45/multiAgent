...
export class ResourceManager {
    ...
    static canAllocate(requiredMemMb: number): boolean {
        ...
        // Fallback for systems where os.totalmem() is low or test override
        const totalMem = snapshot.totalMem;
        const memLimit = this.memOverrideMb ? this.memOverrideMb * 1024 * 1024 : totalMem * this.MAX_MEMORY_USAGE;
        const currentUsage = totalMem - snapshot.freeMem;
