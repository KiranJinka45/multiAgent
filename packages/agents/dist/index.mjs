...
var BaseAgent = class {
  ...
  async promptLLM(system, user, request, signal, strategy) {
    ...
    const modelConfig = await selectModel(taskType, context);
...
// src/judge-agent.ts
import logger2 from "@libs/utils";
var JudgeAgent = class extends BaseAgent {
  ...
  async execute(request) {
    ...
    logger2.info({ tenantId }, "[JudgeAgent] Evaluating intent vs output");
    const systemPrompt = `
...
Respond in JSON format:
{
  ...
  "criticism": ["Point 1", "Point 2"]
    ...
    try {
      ...
      logger2.error({ err }, "[JudgeAgent] Failed to perform intent evaluation");
      return {
        success: false,
        data: null,
        artifacts: [],
...
// src/meta-agent.ts
import logger3 from "@libs/utils";
...
Output strictly valid JSON matching this schema:
{
  ...
  "recommendedTechStack": {
    ...
    "styling": "string"
      ...
      logger3.error({ error }, "MetaAgent analysis failed");
      return {
        success: false,
        data: null,
        artifacts: [],
...
var AgentRegistry = class {
  ...
  async runTaskDirectly(taskType, request, signal, strategy) {
    ...
    try {
      logger5.info({ taskType, executionId: request.context.executionId }, "System dispatching to specialized Agent");
      return await agent.execute(request, signal, strategy);
    } catch (error) {
      logger5.error({ error, taskType }, "Specialized agent crashed during execution");
      return {
        success: false,
        data: null,
        artifacts: [],
...
var KnowledgeService = class {
  ...
  static async getContext(prompt, techStack, limit = 3) {
    try {
      logger6.info({ prompt, techStack }, "[KnowledgeService] Fetching contextual intelligence...");
      const embedding = await EmbeddingsEngine.generate(prompt);
      if (!embedding) {
        logger6.warn("[KnowledgeService] Failed to generate embedding. Proceeding without context.");
        return [];
      }
      ...
      logger6.error({ error }, "[KnowledgeService] Context retrieval failed");
      return [];
    }
  }
  /**
...
${basePrompt}
      ...
      logger6.error({ error }, "[KnowledgeService] Failed to augment prompt with MemoryPlane");
      return basePrompt;
    }
  }
};
