...
async function testRedis() {
    ...
    try {
        ...
        if (val !== 'ok')
            throw new Error(`Read/Write mismatch: ${val}`);
...
async function testWorkerHeartbeat() {
    ...
    try {
        ...
        if (!healthStr)
            throw new Error('No worker heartbeat key found — is the worker running?');
...
async function testMissionLifecycle() {
    const redis = new ioredis_1.default(REDIS_URL, { lazyConnect: true });
    await redis.connect();
    try {
        ...
        if (!stored)
            throw new Error('Mission state not found after write');
        const parsed = JSON.parse(stored);
        if (parsed.status !== 'queued')
            throw new Error(`Expected status 'queued', got '${parsed.status}'`);
        ...
        if (updatedParsed.status !== 'executing')
        ...
        if (finalParsed.status !== 'completed')
            throw new Error('Final state transition failed');
...
async function testPubSub(testMissionId) {
    const pubRedis = new ioredis_1.default(REDIS_URL, { lazyConnect: true });
    const subRedis = new ioredis_1.default(REDIS_URL, { lazyConnect: true });
...
async function testPreviewProxy() {
    const redis = new ioredis_1.default(REDIS_URL);
