    ...
    return (
        <div className="max-w-6xl mx-auto p-6 space-y-12 pb-32">
                ...
                {strategySections.map((section, idx) => (
                    ...
                    >
                        ...
                        <div className="mt-auto pt-4 border-t border-border/50">
                            <span className="text-[10px] uppercase font-bold text-primary tracking-widest block mb-1">Real World Use Case</span>
                            <p className="text-xs text-foreground italic">{section.useCase}</p>
                        </div>
                    </motion.div>
                ))}
            </div>
            ...
            <div id="help" className="mt-20 p-8 rounded-3xl bg-accent/20 border border-border space-y-8 animate-in fade-in duration-700">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-muted-foreground">Everything you need to master your MultiAgent environment.</p>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                    <div className="space-y-3">
                        <h3 className="font-bold text-foreground flex items-center gap-2">
                            <Zap size={16} className="text-yellow-500" /> Getting Started
                        </h3>
                        <p className="text-sm text-muted-foreground">New to MultiAgent? Start by creating a mission in the sidebar. Select a model based on your complexity needs (Flash for speed, Pro for reasoning).</p>
                    </div>
                    <div className="space-y-3">
                        <h3 className="font-bold text-foreground flex items-center gap-2">
                            <Shield size={16} className="text-red-500" /> Account & Security
                        </h3>
                        <p className="text-sm text-muted-foreground">Manage your profile and mission history via the user menu in the bottom left. All data is protected via Enterprise RLS.</p>
                    </div>
                    <div className="space-y-3">
                        <h3 className="font-bold text-foreground flex items-center gap-2">
                            <Layers size={16} className="text-blue-500" /> Tool Integration
                        </h3>
                        <p className="text-sm text-muted-foreground">Use the input bar's 'Tools' menu to trigger Deep Research or Canvas modes. These utilize specialized LLM architectures for high-quality outputs.</p>
                    </div>
                    <div className="space-y-3">
                        <h3 className="font-bold text-foreground flex items-center gap-2">
                            <Users2 size={16} className="text-emerald-500" /> Collaboration
                        </h3>
                        <p className="text-sm text-muted-foreground">Share missions or start group chats via the context menu on any chat item in the sidebar history.</p>
                    </div>
                </div>
            </div>
            ...
            <div className="p-8 rounded-3xl bg-primary/5 border border-primary/20 text-center space-y-4">
                ...
                <h3 className="text-xl font-bold">Mission Ready?</h3>
                <p className="text-muted-foreground max-w-xl mx-auto">
                    These strategies ensure your agents are secure, scalable, and cost-effective.
                    Close this window or select 'New Mission' to begin your next orbit.
                </p>
            </div>
        </div>
    );
