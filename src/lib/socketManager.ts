...
class SocketManager {
    ...
    public addUpdateListener(event: string, listener: (data: any) => void) {
    ...
    private notifyUpdateListeners(event: string, data: any) {
        const listeners = this.updateListeners.get(event);
        if (listeners) {
    ...
    public async connect(options?: SocketManagerOptions): Promise<Socket | null> {
        ...
        try {
            ...
            if (!isHealthy) {
                console.warn(`[SocketManager] Server at ${this.serverUrl} is unreachable. Skipping socket connection.`);
                this.isConnecting = false;
                this.scheduleRetry();
                return null;
            }

            // 2. Clear any pending retries since we are connecting
            ...
            this.socket = io(this.serverUrl, {
                reconnection: false, // We handle reconnection manually via health check
                timeout: 5000,
