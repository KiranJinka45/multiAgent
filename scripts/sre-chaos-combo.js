...
async function runChaosCombo() {
    ...
    for (let loop = 0; loop < LOOPS; loop++) {
        ...
        for (let i = 0; i < CONCURRENCY; i++) {
            loopPromises.push((async () => {
                try {
                    const res = await axios_1.default.post(`${CORE_API_URL}/api/generate`, {
        ...
        if (loop === 1) {
            ...
            try {
                ...
                const workerProcess = (0, child_process_1.exec)('npx tsx -r tsconfig-paths/register apps/worker/src/index.ts', {
    ...
    while (completed < activeMissions.length && (Date.now() - start) < timeout) {
        const statuses = await Promise.all(activeMissions.map(id => db_1.db.mission.findUnique({ where: { id } })));
        ...
        if (completed + failed === activeMissions.length)
            break;
        await new Promise(r => setTimeout(r, 2000));
    }
...
//# sourceMappingURL=sre-chaos-combo.js.map