...
import { contextStorage } from '../packages/db/src/context.ts';
...
async function verifyIsolation() {
    ...
    await contextStorage.run({ requestId: 'test-1', tenantId: tenantA }, async () => {
        const created = await db.executionLog.create({
            data: {
                executionId,
    ...
    await contextStorage.run({ requestId: 'test-2', tenantId: tenantB }, async () => {
        const record = await db.executionLog.findFirst({
            where: { executionId }
        });
    ...
    const globalRecord = await db.executionLog.findFirst({
        where: { executionId }
    });
    ...
    await contextStorage.run({ requestId: 'test-admin', tenantId: 'platform-admin' }, async () => {
        const adminRecord = await db.executionLog.findFirst({
            where: { executionId }
        });
