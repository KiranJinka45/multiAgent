...
async function verifyRBAC() {
    ...
    for (const role of roles) {
        ...
        // Signup
        const signupRes = await axios_1.default.post(`${AUTH_URL}/signup`, {
            email,
            password,
            name: `RBAC ${role}`
        }).catch(e => {
        ...
        if (role !== 'viewer') {
            await src_1.db.user.update({
                where: { id: user.id },
                data: { role }
            });
            // Re-login to get updated token with new role
            const loginRes = await axios_1.default.post(`${AUTH_URL}/login`, { email, password });
    ...
    for (const tc of testCases) {
        const user = tc.role === 'unauthorized' ? null : users[tc.role];
        const headers = user ? { Authorization: `Bearer ${user.token}` } : {};
        console.log(`\n🔍 Testing: ${tc.name}`);
        try {
            const res = await axios_1.default[tc.method](`${GATEWAY_URL}${tc.path}`, tc.body || {}, { headers, timeout: 5000 });
