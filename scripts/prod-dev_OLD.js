...
async function killExistingProcesses() {
    ...
    try {
        ...
        (0, child_process_1.execSync)(`powershell -Command "${psCommand}"`, { stdio: 'ignore' });
...
async function retryTask(taskName, task, maxAttempts = 3, initialDelay = 1000) {
    let attempt = 1;
...
async function bootService(service) {
    log(LOG_PREFIX.BOOT, `Starting service: ${service.name}...`);
    const child = (0, child_process_1.spawn)('pnpm', ['turbo', 'run', 'dev', '--filter', service.filter, '--concurrency', '8'], {
        stdio: 'inherit',
        shell: true,
...
async function main() {
    ...
    await retryTask('Prisma Client Generation', async () => {
        ...
        const prismaProc = (0, child_process_1.spawn)('npx.cmd', ['prisma', 'generate', '--schema', 'packages/db/src/schema.prisma'], {
            stdio: 'inherit',
            shell: true,
...
//# sourceMappingURL=prod-dev_OLD.js.map