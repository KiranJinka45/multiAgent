...
async function main() {
    ...
    files.slice(0, 5).forEach((f) => info(`  ${f.path}`));
    // ── STEP 2: ProjectMemory ────────────────────────────
    section('STEP 2: ProjectMemory Service');
    const { projectMemory } = await Promise.resolve().then(() => __importStar(require('./src/lib/project-memory')));
    ...
    const memory = await projectMemory.initializeMemory(chosenProject.id, { framework: 'nextjs', styling: 'tailwind', backend: 'api-routes', database: 'supabase' }, mockFiles);
    ...
    const { PlannerAgent } = await Promise.resolve().then(() => __importStar(require('./src/agents/planner-agent')));
    ...
    const { ChatEditAgent } = await Promise.resolve().then(() => __importStar(require('./src/agents/chat-edit-agent')));
    ...
    if (!editResult.success || !editResult.data) {
        ...
        er.patches?.forEach(p => {
            ...
            if (p.content)
                info(`    Content preview: ${p.content.substring(0, 100).replace(/\n/g, ' ')}...`);
        });
        if (er.newFeatures?.length)
            info(`New features tagged: ${er.newFeatures.join(', ')}`);
    ...
    try {
        ...
        if (res.status === 200 && body.success) {
            pass(`API 200 OK — "${body.explanation}"`);
            (body.patches || []).forEach((p) => info(`  [${p.action}] ${p.path}`));
        }
...
//# sourceMappingURL=test-chat-edit.js.map