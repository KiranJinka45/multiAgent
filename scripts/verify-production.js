...
async function runTests() {
    ...
    try {
        const res = await axios_1.default.get(`${CORE_API_URL}/health`);
    ...
    const testRequestId = `test-trace-${(0, uuid_1.v4)()}`;
    try {
        const res = await axios_1.default.get(`${GATEWAY_URL}/api/missions`, {
    ...
    try {
        ...
        await axios_1.default.post(`${GATEWAY_URL}/api/missions/start`, { title: 'Invalid Mission' });
    }
    catch (e) {
    ...
    try {
        ...
        await axios_1.default.get(`${GATEWAY_URL}/api/projects`, {
    ...
    try {
        const res = await axios_1.default.get(`${CORE_API_URL}/metrics`);
        if (res.status === 200) {
