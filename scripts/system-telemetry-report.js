...
const reliability_monitor_1 = require("../packages/utils/src/services/reliability-monitor");
const redis_1 = require("../packages/utils/src/services/redis");
async function main() {
    console.log('\n╔══════════════════════════════════════════════════════════╗');
    ...
    try {
        ...
        const stats = await reliability_monitor_1.ReliabilityMonitor.getStats();
        ...
        const executionLogs = await src_1.db.executionLog.groupBy({
            by: ['success'],
            _count: { success: true }
        });
        let successTasks = 0;
        ...
        executionLogs.forEach(group => {
            ...
            if (group.success)
                successTasks += group._count.success;
        });
        ...
        // 3. Latency Audit
        const avgLatency = await src_1.db.executionLog.aggregate({
            _avg: { latency: true }
        });
        console.log(`\n--- Performance Audit ---`);
...
main();
//# sourceMappingURL=system-telemetry-report.js.map