...
async function runSmokeTest() {
    ...
    try {
        ...
        const buildResponse = await axios_1.default.post(`${GATEWAY_URL}/api/builds`, {
        ...
        while (status !== 'completed' && status !== 'failed' && attempts < 30) {
            const statusRes = await axios_1.default.get(`${GATEWAY_URL}/api/core/builds/660e8400-e29b-41d4-a716-446655440022/status`, {
                headers: { Authorization: `Bearer ${JWT_TOKEN}` }
            });
