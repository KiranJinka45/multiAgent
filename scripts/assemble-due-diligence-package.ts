...
### 4. Pilot Environment Verification
- **Current Status:** \`[PILOT VERIFIED]\`
- **Description:** Deploying the control plane to the first live enterprise tenant under real-world traffic and monitoring (validated under local pilot environment).
- **Environment Assumption:** Requires a low-risk tenant, real telemetry, real incident-handling, and a 30-day observation window.
- **Verification Path:** Completed Local Kubernetes Pilot Validation. Deployed the ZTAN control plane to local Kubernetes ztan-pilot namespace, verifying gateway, auth-service, worker, and databases under strict network policy isolation. Evidence: \`pilot-deployment-report.json\`.
`;
...
### 4. Pilot Environment Verification Evidence
