...
async function inspectStream() {
    try {
        ...
        for (const [id, fields] of entries) {
            ...
            if (data.payload) {
                try {
                    allParsed.push(JSON.parse(data.payload));
                }
                catch (e) { }
            }
            else if (data.data) {
                try {
                    allParsed.push(JSON.parse(data.data));
                }
                catch (e) { }
            }
            else {
        ...
        const packageJsonEvent = allParsed.find(e => e.path === 'package.json' ||
...
inspectStream();
//# sourceMappingURL=inspect-stream.js.map