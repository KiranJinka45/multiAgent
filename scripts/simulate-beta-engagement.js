...
async function simulateEngagement() {
    ...
    const tester01 = await db_1.db.user.findFirst({ where: { email: 'beta-test-01@example.com' } });
    const tester02 = await db_1.db.user.findFirst({ where: { email: 'beta-test-02@example.com' } });
    ...
    await db_1.db.betaFeedback.create({
        data: {
            tenantId: tester01.tenantId,
            userId: tester01.id,
            type: 'IMPROVEMENT',
...
simulateEngagement();
//# sourceMappingURL=simulate-beta-engagement.js.map