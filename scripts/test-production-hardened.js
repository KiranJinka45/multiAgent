...
const infra_provisioner_1 = require("../apps/api/services/devops/infra-provisioner");
const memory_service_1 = require("../packages/memory/memory-service");
const audit_logger_1 = require("../packages/utils/services/audit-logger");
const anchoring_service_1 = require("../packages/utils/services/anchoring-service");
const sandbox_runner_1 = require("../apps/sandbox-runtime/sandbox-runner");
async function runVerification() {
    ...
    const meta1 = await infra_provisioner_1.InfraProvisioner.provisionResources(projectId);
    const meta2 = await infra_provisioner_1.InfraProvisioner.provisionResources(projectId);
    ...
    const anchorHash = await anchoring_service_1.AnchoringService.anchorChain(tenantId);
    if (anchorHash) {
        const isValid = await anchoring_service_1.AnchoringService.verifyAgainstAnchor(tenantId, anchorHash);
    ...
    const res = await sandbox_runner_1.SandboxRunner.execute('curl https://google.com', [], {
        cwd: process.cwd(),
    ...
    await memory_service_1.MemoryService.store({
        type: 'error',
        content: 'Fixing a null pointer exception in the auth handler',
        projectId: '00000000-0000-0000-0000-000000000000',
        metadata: { tag: 'semantic-test' }
    }, tenantId);
    const matches = await memory_service_1.MemoryService.retrieve('How to handle null pointers in authentication?', tenantId);
...
//# sourceMappingURL=test-production-hardened.js.map