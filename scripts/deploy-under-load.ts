...
async function deployUnderLoad() {
    ...
    const token = jwt.sign({ id: 'stress-user', roles: ['admin'], tenantId: 'system' }, JWT_SECRET);
    ...
    while (completed.size + failed.size < missionIds.length && (Date.now() - start < timeout)) {
        for (const mid of missionIds) {
            if (completed.has(mid) || failed.has(mid)) continue;

            const mission = await db.mission.findUnique({ where: { id: mid } });
