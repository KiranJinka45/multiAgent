...
async function runTimelineScopingTest() {
  ...
  const submissionPromises = Array.from({ length: PARALLEL_JOBS }).map((_, i) => 
  ...
  for (const missionId of missionIds) {
    try {
      const res = await axios.get(`${API_URL}/missions/${missionId}/timeline`, { headers });
      const events = res.data.events || [];
      
      const foreignEvents = events.filter((e: any) => e.missionId && e.missionId !== missionId);
