...
async function verifyGovernance() {
    ...
    const token = jwt.sign({ id: userId, tenantId, roles: ['user'] }, JWT_SECRET, { expiresIn: '1h' });
    const headers = { Authorization: `Bearer ${token}` };

    // 2. Test Concurrency Limit (Free Plan = 2)
    console.log('\n📡 Testing concurrency limit (expecting 2 allowed/504 and 1 rejected/403)...');
    
    const concurrencyBurst = [1, 2, 3].map(i => 
        axios.post(`${API_URL}/builds`, { 
    ...
    burstResults.forEach((res, i) => {
        ...
        if (res.status === 403) forbiddenCount++;
        else if (res.status === 504 || res.status === 502 || res.status === 200 || res.status === 201) allowedCount++;
    });
    ...
    const burstPromises = Array.from({ length: 120 }).map((_, i) => 
        axios.post(`${API_URL}/builds`, {
            prompt: `Rate Test #${i}`,
            ...
            if (e.response?.status === 429) rejectedCount++;
            else if (e.response?.status === 403) concurrencyCount++;
...
verifyGovernance().catch(console.error).finally(() => db.$disconnect());
