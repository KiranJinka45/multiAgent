...
export const RuntimeHeartbeat = {
     ...
     */
    async publish(projectId: string, pids: number[], ports: number[]): Promise<void> {
        await redis.setex(
     ...
     */
    startLoop(projectId: string, pids: number[], ports: number[]): void {
        this.stopLoop(projectId); // Idempotent

        const timer = setInterval(async () => {
            try {
                await this.publish(projectId, pids, ports);
                logger.debug({ projectId, pids, ports }, '[Heartbeat] Published');
            } catch (err) {
        ...
        logger.info({ projectId, pids, ports }, '[Heartbeat] Loop started');
    },
     ...
     */
    async getLast(projectId: string): Promise<{ pids: number[]; ports: number[]; ts: string } | null> {
