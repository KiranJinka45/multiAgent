...
import { type SupabaseClient, type User } from '@packages/supabase';
...
export const SidebarProfile = ({ user, supabase, hideLabels }: SidebarProfileProps) => {
    ...
    return (
        <div className="relative pt-4 border-t border-border" ref={profileRef}>
            ...
            >
                ...
                {!hideLabels && <ChevronDown size={14} className={`text-muted-foreground transition-transform duration-300 ${isOpen ? 'rotate-180' : ''}`} />}
            </div>
        </div>
    );
};
