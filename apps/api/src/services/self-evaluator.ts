...
Output strictly valid JSON:
{
  ...
  "criticalIssues": ["List of blockers that must be fixed, empty if none"]
            ...
            const filesSummary = input.files.slice(0, 20).map((f: { path: string, content: string }) =>
...
${filesSummary}`;
            ...
            const { result, tokens } = await (this as any).promptLLM(system, userPrompt, 'llama-3.3-70b-versatile', signal);
