...
export class OperationalAuditEngine {
  ...
  public verify(decisionId: string, predicted: number, actual: number, confidence: number): AuditRecord {
    ...
    logger.info({ 
        ...
        roiError, 
        roiAccuracy 
    }, '[OPERATIONAL-AUDIT] ROI Verification Details');
    ...
    logger.info({ decisionId, brier, regret, roiAccuracy }, '[OPERATIONAL-AUDIT] Decision verified and signed');
    return finalRecord;
  }
  ...
  public async reset() {
    ...
    try {
        ...
        logger.info('[OPERATIONAL-AUDIT] Persistent audit logs cleared');
    } catch (e) {
        logger.error({ err: e }, '[OPERATIONAL-AUDIT] Failed to clear persistent logs');
    }
  }
}

export const operationalAudit = new OperationalAuditEngine();
