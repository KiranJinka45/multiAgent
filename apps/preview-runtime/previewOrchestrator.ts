...
import { PreviewRegistry, RuntimeStatus } from '@registry/previewRegistry';
import { RuntimeMetrics } from './runtimeMetrics';
import { PreviewRuntimePool } from './previewRuntimePool';
...
export const PreviewOrchestrator = {
    ...
    async start(projectId: string, executionId: string, userId?: string): Promise<string> {
        ...
        try {
            ...
            // 4. Phase 2: Dual-mode spawn with Pool Optimization ───────
            if (RUNTIME_MODE === 'docker') {
                logger.info({ projectId, port }, '[PreviewOrchestrator] Docker mode: checking for warm container');
                
                // --- POOL OPTIMIZATION ---
                const warmContainer = await PreviewRuntimePool.checkout(projectId, port);
                if (warmContainer) {
                    logger.info({ projectId, containerId: warmContainer.containerId }, '[PreviewOrchestrator] Using warm container from pool');
                    
                    // --- LAYER-20 HOT INJECTION ---
                    ...
                    pid = parseInt(warmContainer.containerId, 16) || 0; // Assuming containerId can be used as a pseudo-pid
            ...
            // Artifact validation gate (L20 Stability Hack)
            const validation = await ArtifactValidator.validate(projectId);
            if (!validation.valid) {
                const missing = validation.missingFiles?.join(', ') || 'critical files';
                logger.error({ projectId, missing }, '[PreviewOrchestrator] Artifact validation failed');
                throw new Error(`Sandbox artifacts (${missing}) were not found on disk. Build probably failed silently.`);
            }
