...
if (node_cluster_1.default.isPrimary) {
    ...
    observability_1.logger.info(`[Gateway] Primary Cluster Manager ${process.pid} is running`);
    observability_1.logger.info(`[Gateway] Forking ${maxWorkers} worker processes (Detected Cores: ${rawCpuCount})...`);
    ...
    node_cluster_1.default.on('exit', (worker, code, signal) => {
        observability_1.logger.warn({ pid: worker.process.pid, code, signal }, `[Gateway] Worker proxy died! Spawning replacement...`);
        node_cluster_1.default.fork();
    });
}
else {
    // Dynamic import of the server logic only in workers
    Promise.resolve().then(() => __importStar(require('./server.js'))).then(({ startGatewayServer }) => {
        startGatewayServer().catch(err => {
            observability_1.logger.error({ err }, '[Gateway] Failed to start worker server');
            process.exit(1);
        });
    }).catch(err => {
